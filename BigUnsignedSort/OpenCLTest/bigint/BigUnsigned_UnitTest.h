...
void testBigUnsignedAddition() {
	...
	struct BigUnsigned result = addBU(createBU((unsigned long)(4294967295)), createBU(1));	
...
void testBigUnsignedSubtraction() {
	...
	struct BigUnsigned result = subtractBU(bigNumber, createBU(1));
...
void testBigUnsignedBlockAccessors(){
	...
	//getBUBlock
	struct BigUnsigned bu = createBU(1);
...
void testBigUnsignedBitAccessors(){
	...
	struct BigUnsigned bu = createBU(7);
...
void testBigUnsignedAnd(){
	...
	struct BigUnsigned x = createBU(12);
	x.len++;
	x.blk[1] = 12;
	struct BigUnsigned y = createBU(9);
	y.len++;
	y.blk[1] = 9;
...
void testBigUnsignedShiftLeft(){
	...
	struct BigUnsigned x = createBU(2);
...
void testBigUnsignedComparison(){
	...
	struct BigUnsigned x = createBU(1);
	struct BigUnsigned y = createBU(1);
	y.len++;
	y.blk[1] = 1;
	struct BigUnsigned z = createBU(1);
	struct BigUnsigned w = createBU(0);
