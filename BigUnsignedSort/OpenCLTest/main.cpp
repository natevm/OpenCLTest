...
#pragma region Creating_Program	
	std::cout << "Creating a program kernel: " << std::endl;
	...
	kernel = clCreateKernel(program, "StreamScan", &error);
	std::cout << "Creating \"StreamScan\" kernel" << std::endl;
	...
	cl_mem inputBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE| CL_MEM_COPY_HOST_PTR,
		sizeof (int)* (testDataSize),
		input.data(), &error);
	std::cout << testDataSize / workgroupSize << std::endl;
	cl_mem intermediateBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
		sizeof (int)* (testDataSize/workgroupSize),
	...
	clSetKernelArg(kernel, 1, sizeof(cl_mem), &intermediateBuffer);
	clSetKernelArg(kernel, 2, workgroupSize * sizeof(cl_int), NULL);
	clSetKernelArg(kernel, 3, workgroupSize * sizeof(cl_int), NULL);
