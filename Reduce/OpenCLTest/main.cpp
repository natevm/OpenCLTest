...
#pragma region Creating_Program	
	...
	cl_kernel kernel = clCreateKernel(program, "addReduce", &error);
	CheckError(error);
	std::cout << "SUCCESS" << std::endl;
	...
	clSetKernelArg(kernel, 1, testDataSize * sizeof(cl_float), NULL);
	clSetKernelArg(kernel, 2, sizeof (int), &testDataSize);
	clSetKernelArg(kernel, 3, sizeof (cl_mem), &resultBuffer);
