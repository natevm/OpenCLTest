...
#pragma region Creating_Program	
	...
	cl_kernel kernel;
	if (hillesSteel) {
		kernel = clCreateKernel(program, "HillesSteelScan", &error);
		std::cout << "Hilles Steel Scan Selected" << std::endl;
	}
	else {
		kernel = clCreateKernel(program, "BlellochScan", &error);
		std::cout << "Blelloch Scan Selected" << std::endl;
	}
	CheckError(error);
	std::cout << "SUCCESS" << std::endl;
	...
	static const size_t testDataSize = 1 << 4;
	...
	clSetKernelArg(kernel, 2, testDataSize * sizeof(cl_float), NULL);
	if (hillesSteel){
		clSetKernelArg(kernel, 3, testDataSize * sizeof(cl_float), NULL);
	}
	else {
		clSetKernelArg(kernel, 3, sizeof(cl_uint), &testDataSize);
	}
	...
	if (hillesSteel) {
		...
		const size_t globalWorkSize[] = { testDataSize/2, 0, 0 };
		CheckError(clEnqueueNDRangeKernel(queue, kernel, 1,
			nullptr,
			globalWorkSize,
			nullptr,
